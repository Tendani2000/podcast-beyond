...
function Show () {
    ...
    const handleAddToFavourites = (showId) => {
        if (!favourites.find((favourite) => favourite.id === showId)) {
            const showToAdd = shows.find((show) => show.id === showId);
            setFavourites ((prevFavourites) => [...prevFavourites, showToAdd]);
        };
    }
            ...
            <ul>
                {sortedShows.map(show => (
                    <li key={show.id}>
                        ...
                        <div className="add-favourites" onClick={() => handleAddToFavourites(show.id)}>
                        <svg xmlns="http://www.w3.org/2000/svg" 
                            height="1.25em" 
                            fill="currentColor"
                            viewBox="0 0 512 512">
